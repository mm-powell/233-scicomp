...
int Grid2D::signum( double val ) {
    if ( val > 0.02*dx ) {return 1;}
    else if ( val < -0.02*dx ) {return -1;}
    else {return 0;}
}
...
double Grid2D::dx_forward(std::vector<double> &function,int n)
{
    ...
    //else if ( x_from_n(n) == xmin ) {return 0.0;}
...
// Number 6
double Grid2D::sec_dx(std::vector<double> &function,int n)
{
